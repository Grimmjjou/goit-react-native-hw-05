...
const LoginScreen = ({ navigation }) => {
  ...
  const register = () => {
    if (!mail || !password) { alert("Заповніть всі поля!!!"); return }
          ...
          <KeyboardAvoidingView behavior={Platform.OS == "ios" ? "padding" : "height"} style={styles.containerKeyB} >
            <View onPress={() => {
              ...
              <TouchableOpacity style={{
                ...styles.loginLink, ...Platform.select({ ios: { marginBottom: isShowKeyboard ? -115 : 45, }, android: { marginBottom: isShowKeyboard ? -105 : 45, } })

              }} activeOpacity={0.5} onPress={() => navigation.navigate("Registratione")}>
